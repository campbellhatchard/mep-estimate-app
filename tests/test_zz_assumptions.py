...
def test_assumptions_are_revision_scoped_audited_locked_and_carried_forward():
...
def test_cip_range_factors_accept_percent_values_and_store_decimal_factors():
    with TestClient(app) as client:
        login(client)
        rid = create_cip(client)
        response = client.post(
            f'/estimate/{rid}',
            data={'low_factor': '12.5', 'high_factor': '30'},
        ...
        assert "raw * 10000" in script.text
