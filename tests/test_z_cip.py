...
def test_product_chooser_and_cip_default_workbook_golden_scenario():
    with TestClient(app) as client:
        ...
        with SessionLocal() as db:
            ...
            # Approved workbook default: 239 customer estimate hours at $250/hour, 1.12 months.
...
def test_cip_desktop_and_mobile_baseline_small_change_factors_are_intentionally_different():
    with TestClient(app) as client:
        ...
        with SessionLocal() as db:
            ...
            assert d.base_solution_test == 0.01875
            assert m.base_hours == 1
            assert m.base_solution_test == 0.0075
...
def test_cip_very_complex_custom_desktop_uses_resolved_80_hour_rule():
    with TestClient(app) as client:
        ...
        with SessionLocal() as db:
            ...
            assert line.base_hours == 80
            assert line.unit_testing == 12
            assert line.base_solution_test == 20
            assert line.total_effort == 112
...
def test_cip_label_one_and_rest_multi_consumer_rules():
    with TestClient(app) as client:
        ...
        with SessionLocal() as db:
            ...
            assert label_line.definition == "Label 1"
            assert label_line.base_hours == 2
            assert label_line.base_solution_test == 0.4
            assert rest_line.base_hours == 8
            assert rest_line.application_integration_hours == 8  # first app 4 + two additional at 2 each
            assert rest_line.base_solution_test == 1.6
    ...
    with TestClient(app) as client:
        ...
        with SessionLocal() as db:
            ...
            for method in ["LDAP", "Okta", "SAML"]:
                ...
                sso = next(x for x in lines if x.key == "BUILD_SSO")
                assert sso.standard_hours == 16
